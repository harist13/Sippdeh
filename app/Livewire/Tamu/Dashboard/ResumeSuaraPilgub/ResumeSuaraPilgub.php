...
namespace App\Livewire\Tamu\Dashboard\ResumeSuaraPilgub;
...
class ResumeSuaraPilgub extends Component
{
     ...
     public function exportPdf()
    {
        try {
            ...
            $kabupaten = Kabupaten::find(session('Tamu_kabupaten_id'));
    ...
    private function getKelurahanTable()
    {
        ...
        return view('Tamu.dashboard.resume-suara-pilgub.livewire', compact('suara', 'paslon', 'scope'));
    }
    ...
    private function fillSelectedKabupaten()
    {
        $this->selectedKabupaten = Kabupaten::query()
            ->whereHas('provinsi', fn (Builder $builder) => $builder->whereId(session('Tamu_provinsi_id')))
            ->pluck('id')
            ->all();
    }
