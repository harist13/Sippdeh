<div>
        ...
        <div class="bg-white p-4">
            <div class="container mx-auto">
                <div class="flex flex-col gap-5 lg:flex-row lg:space-x-2 lg:items-center lg:justify-between">
                    ...
                    {{-- Cari dan Filter --}}
                    @include('Tamu.resume.pilwali.per-wilayah.export-search-filter')
                </div>
            </div>
        </div>
            ...
            <div class="inline-block min-w-full align-middle">
                <div class="overflow-hidden border-b border-gray-200 shadow sm:rounded-lg relative">
                    ...
                    @if (in_array('TPS', $includedColumns))
                        @include("Tamu.resume.pilwali.per-wilayah.wilayah-tables.tps-table", compact('suara', 'paslon', 'includedColumns'))
                    @else
                        @if (!empty($selectedKelurahan))
                            @include("Tamu.resume.pilwali.per-wilayah.wilayah-tables.kelurahan-table", compact('suara', 'paslon', 'includedColumns'))
                        @elseif (!empty($selectedKecamatan))
                            @include("Tamu.resume.pilwali.per-wilayah.wilayah-tables.kecamatan-table", compact('suara', 'paslon', 'includedColumns'))
                        @elseif (!empty($selectedKabupaten))
                            @include("Tamu.resume.pilwali.per-wilayah.wilayah-tables.kabupaten-table", compact('suara', 'paslon', 'includedColumns'))
                        @endif
                    @endif
                </div>
            </div>
    ...
    <!-- Filter Pilgub Modal -->
    @include(
        'Tamu.resume.pilwali.per-wilayah.filter-modal',
        compact(
            'selectedKecamatan',
            'selectedKelurahan',
            'includedColumns',
