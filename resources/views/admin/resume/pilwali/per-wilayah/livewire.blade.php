        ...
        <div class="bg-white p-4">
            <div class="container mx-auto">
                <div class="flex flex-col gap-5 lg:flex-row lg:space-x-2 lg:items-center lg:justify-between">
                    ...
                    {{-- Cari dan Filter --}}
                    @include('admin.resume.pilwali.per-wilayah.export-search-filter')
                </div>
            </div>
        </div>
            ...
            <div class="inline-block min-w-full align-middle">
                <div class="overflow-hidden border-b border-gray-200 shadow sm:rounded-lg relative">
                    ...
                    @if (in_array('TPS', $includedColumns))
                        @include("admin.resume.pilwali.per-wilayah.wilayah-tables.tps-table", compact('suara', 'paslon', 'includedColumns'))
                    @else
                        @if (!empty($selectedKelurahan))
                            @include("admin.resume.pilwali.per-wilayah.wilayah-tables.kelurahan-table", compact('suara', 'paslon', 'includedColumns'))
                        @elseif (!empty($selectedKecamatan))
                            @include("admin.resume.pilwali.per-wilayah.wilayah-tables.kecamatan-table", compact('suara', 'paslon', 'includedColumns'))
                        @elseif (!empty($selectedKabupaten))
                            @include("admin.resume.pilwali.per-wilayah.wilayah-tables.kabupaten-table", compact('suara', 'paslon', 'includedColumns'))
                        @endif
                    @endif
                </div>
            </div>
