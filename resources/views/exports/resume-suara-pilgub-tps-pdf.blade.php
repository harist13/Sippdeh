...
<head>
    ...
    <title>Resume Suara Pemilihan Gubernur Per TPS</title>
      <style>
        @font-face {
            ...
            <tbody>
                @forelse($data as $index => $item)
                    <tr>
                        ...
                        @if(in_array('PROVINSI', $includedColumns))
                            <td class="text-left">{{ $item->tps?->kelurahan?->kecamatan?->kabupaten?->provinsi?->nama ?? '-' }}</td>
                        @endif
                        @if(in_array('KABUPATEN/KOTA', $includedColumns))
                ...
                @empty
                    <tr>
                        <td colspan="{{ 8 + count($paslon) + (in_array('PROVINSI', $includedColumns) ? 1 : 0) + (in_array('KABUPATEN/KOTA', $includedColumns) ? 1 : 0) + (in_array('KECAMATAN', $includedColumns) ? 1 : 0) + (in_array('KELURAHAN', $includedColumns) ? 1 : 0) + (in_array('TPS', $includedColumns) ? 1 : 0) }}" class="text-center">
                            Data tidak tersedia.
                        </td>
                    </tr>
                @endforelse
