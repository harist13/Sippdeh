<div>
        ...
        <div class="bg-white p-4">
            <div class="container mx-auto">
                <div class="flex flex-col gap-5 lg:flex-row lg:space-x-2 lg:items-center lg:justify-between">
                    ...
                    {{-- Cari dan Filter --}}
                    @include('operator.resume.pilbup.per-wilayah.export-search-filter')
                </div>
            </div>
        </div>
            ...
            <div class="inline-block min-w-full align-middle">
                <div class="overflow-hidden border-b border-gray-200 shadow sm:rounded-lg relative">
                    ...
                    @if (in_array('TPS', $includedColumns))
                        @include("operator.resume.pilbup.per-wilayah.wilayah-tables.tps-table", compact('suara', 'paslon', 'includedColumns'))
                    @else
                        ...
                        @elseif (!empty($selectedKabupaten))
                            @include("operator.resume.pilbup.per-wilayah.wilayah-tables.kabupaten-table", compact('suara', 'paslon', 'includedColumns'))
                        @endif
                    @endif
                </div>
            </div>
    ...
    <!-- Filter Pilgub Modal -->
    @include(
        'operator.resume.pilbup.per-wilayah.filter-modal',
        compact(
            'selectedKecamatan',
            'selectedKelurahan',
            'includedColumns',
