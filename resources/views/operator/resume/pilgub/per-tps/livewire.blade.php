<div>
		...
		<div class="bg-white p-4 rounded-t-[20px]">
			<div class="container mx-auto">
				<div class="flex flex-col gap-5 lg:flex-row lg:space-x-2 lg:items-center lg:justify-between">
					...
					{{-- Cari dan Filter --}}
					@include('operator.resume.pilgub.per-tps.export-search-filter')
				</div>
			</div>
		</div>
		...
		<div class="overflow-x-auto -mx-4 sm:mx-0">
			<div class="inline-block min-w-full align-middle">
				<div class="overflow-hidden border-b border-gray-200 shadow sm:rounded-lg relative">
					...
					<div class="px-4">
						@include('operator.resume.pilgub.per-tps.table', compact('tps', 'paslon', 'includedColumns'))
					</div>
				</div>
			</div>
		</div>
	...
	{{-- Filter Pilgub Modal --}}
	@include(
		'operator.resume.pilgub.per-tps.filter-modal',
